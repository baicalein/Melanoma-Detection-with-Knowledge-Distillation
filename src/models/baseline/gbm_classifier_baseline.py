...
OUT_DIR_TBLS = ROOT / "artifacts" / "tbls" / "01_baselines" / "gbm"
OUT_DIR_TBLS.mkdir(parents=True, exist_ok=True)

LOG_PATH = ROOT / "models" / "logs" / "gbm.log"
...
class ImageCSVDataSet(Dataset):
    ...
    def __init__(
        self, df: pd.DataFrame, transform: Optional[transforms.Compose] = None
    ):
...
# Augmentations for aggregation (match other baselines)
...
def extract_features_with_augmentations(
    ...
    for i, t in enumerate(transforms_list):
        logger.info(
            "Extracting features for augmentation %d/%d", i + 1, len(transforms_list)
        )
...
def _cache_paths(
    debug_subset: Optional[int], augmentations: bool = False
    ...
    if augmentations:
        suffix += "_aug"
    train_cache = features_dir / f"gbm_features_train{suffix}.npz"
    val_cache = features_dir / f"gbm_features_val{suffix}.npz"
...
def train_baseline_and_tune(
    ...
    """Train baseline GBM and RandomizedSearchCV-tuned GBM. Save artifacts and metrics."""
    results: Dict[str, Any] = {"timestamp": time.time()}

    # Baseline GBM (HistGradientBoosting)
    baseline = HistGradientBoostingClassifier(max_iter=200, random_state=random_seed)
    baseline.fit(X_train, y_train)
    baseline_train = evaluate_model(baseline, X_train, y_train)
    baseline_val = evaluate_model(baseline, X_val, y_val)
    baseline_path = out_dir / "gbm_baseline.joblib"
    joblib.dump(
        {
            "model": baseline,
            "metrics_train": baseline_train,
            "metrics_val": baseline_val,
        },
        baseline_path,
    )
    logger.info("Saved baseline GBM to %s", baseline_path)
    results["baseline"] = {
    ...
    gbm_for_search = HistGradientBoostingClassifier(random_state=random_seed)
    ...
    logger.info("Starting RandomizedSearchCV for GBM")
    ...
    tuned_path = out_dir / "gbm_tuned_randomsearch.joblib"
    joblib.dump(
        {
            "model": tuned,
            "best_params": rs.best_params_,
            "metrics_train": tuned_train,
            "metrics_val": tuned_val,
        },
        tuned_path,
    )
    logger.info("Saved tuned GBM to %s", tuned_path)
    results["tuned"] = {
        "path": str(tuned_path),
        "best_params": rs.best_params_,
        "train": tuned_train,
    ...
    # save CV results
    try:
        cvres_path = out_dir / "gbm_randomsearch_cv_results.json"
        ...
        with open(cvres_path, "w") as fh:
            json.dump(cv, fh)
        logger.info("Saved GBM CV results to %s", cvres_path)
...
def main(
    ...
    """Main routine for GBM baseline + tuning using ResNet features."""
    if device is None:
        if (
    ...
    if debug_subset:
        ...
        train_df = train_df.sample(
            n=min(n, len(train_df)), random_state=RANDOM_SEED
        ).reset_index(drop=True)
        val_df = val_df.sample(
            n=min(n, len(val_df)), random_state=RANDOM_SEED
        ).reset_index(drop=True)
    ...
    if train_cache.exists():
        ...
        if augmentations:
            X_train, y_train = extract_features_with_augmentations(
                feat_model,
                train_df,
    ...
    if val_cache.exists():
        ...
        if augmentations:
            X_val, y_val = extract_features_with_augmentations(
                feat_model,
                val_df,
                AUGMENTATION_TRANSFORMS,
                device=device,
    ...
    logger.info(
        "Features shapes: X_train=%s y_train=%s | X_val=%s y_val=%s",
        X_train.shape,
        y_train.shape,
        X_val.shape,
        y_val.shape,
    ...
    artifacts = train_baseline_and_tune(
        X_train, y_train, X_val, y_val, random_seed=RANDOM_SEED, out_dir=OUT_DIR_TBLS
    )

    summary_path = OUT_DIR_TBLS / "gbm_artifacts_summary.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GBM (HistGradientBoosting) baseline on ResNet features."
    )
    parser.add_argument(
        "--debug_subset", type=int, default=None, help="Quick debug subset size"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--augmentations",
        action="store_true",
        help="Aggregate features from augmentations",
    )
